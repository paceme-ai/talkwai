            ...
            <div>
                ...
                <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                    <header className="mx-auto max-w-6xl pb-6">
                        <h2 className="text-xl md:text-2xl font-extrabold tracking-tight">What Do Missed Calls Cost You?</h2>
                    </header>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <InputNumber label="Daily missed calls" value={missedCallsPerDay} onChange={setMissedCallsPerDay} min={0} step={1} />
                        <InputNumber label="Booking rate (%)" value={bookingRatePct} onChange={setBookingRatePct} min={0} max={100} step={1} />
                        <InputNumber label="Average job value ($)" value={avgJobValue} onChange={setAvgJobValue} min={0} step={10} />
                        <InputNumber label="Work days / month" value={operatingDaysPerMonth} onChange={setOperatingDaysPerMonth} min={1} max={31} step={1} />
                        <InputNumber label="Average call length (min)" value={avgCallMinutes} onChange={setAvgCallMinutes} min={0} step={0.5} />
                    </div>
