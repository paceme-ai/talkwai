...
if (!process.env.NEXT_PUBLIC_INSTANT_APP_ID) {
  console.error('❌ NEXT_PUBLIC_INSTANT_APP_ID environment variable is required');
  process.exit(1);
}

if (!process.env.INSTANT_ADMIN_TOKEN) {
  console.error('❌ INSTANT_ADMIN_TOKEN environment variable is required');
  console.log('💡 Make sure to run this script in the context where environment variables are available');
  process.exit(1);
}
...
async function testCallMetadata() {
  ...
  try {
    ...
    if (result.tasks && result.tasks.length > 0) {
      ...
      console.log('  - Start time:', new Date(task.callStartTime).toISOString());
    ...
    if (allTasks.tasks && allTasks.tasks.length > 0) {
      ...
      callTasks.forEach((task, index) => {
        console.log(`  ${index + 1}. Call ID: ${task.callId || 'N/A'}, Status: ${task.callStatus || 'N/A'}, Duration: ${task.callDuration || 'N/A'}s`);
      });
    }
    ...
    console.log('✅ Successful calls found:', successfulCalls.tasks?.length || 0);
